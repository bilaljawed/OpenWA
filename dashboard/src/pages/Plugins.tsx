...
export default function Plugins() {
  ...
  const [engineConfig, setEngineConfig] = useState<EngineConfig>({
    ...
    browserArgs: '--no-sandbox --disable-setuid-sandbox --disable-dev-shm-usage --disable-accelerated-2d-canvas --no-first-run --no-zygote --disable-gpu',
  });
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      await pluginsApi.updateConfig(configPlugin.id, {
        ...
        puppeteerArgs: engineConfig.browserArgs.trim().split(/\s+/).filter(Boolean),
      });
