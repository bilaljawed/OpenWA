...
export function Sessions() {
  ...
  const fetchQR = useCallback(async (sessionId: string) => {
    try {
      ...
      // QR not ready yet (Chrome still starting) — keep modal open with spinner
      if (msg.includes('not ready') || msg.includes('Please wait')) return;
      setQrData(null);
      currentSessionName.current = '';
      fetchSessions();
    }
  ...
  useEffect(() => {
    if (qrData) {
      ...
      // Immediate attempt when modal opens (e.g. after start), then every 5 s
      if (!qrData.qrCode) fetchQR(qrData.sessionId);
  ...
  const handleStart = async (id: string) => {
    ...
    try {
      ...
      // Open QR modal in loading state immediately; polling will populate the code when Chrome is ready
  ...
  const handleShowQR = async (id: string) => {
    ...
    try {
      ...
      if (msg.includes('already authenticated')) {
        // Session authenticated successfully — close any open QR modal and refresh
        setQrData(null);
        void fetchSessions();
        return;
      }
