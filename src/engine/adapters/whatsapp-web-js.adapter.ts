...
export class WhatsAppWebJsAdapter extends EventEmitter implements IWhatsAppEngine {
  ...
  async initialize(callbacks: EngineEventCallbacks): Promise<void> {
    ...
    try {
      ...
      // Chromium 148+ initialises Crashpad early using $HOME/.config.
      // The 'openwa' system user has HOME=/nonexistent which doesn't exist,
      // causing chrome_crashpad_handler to be invoked without --database → abort.
      // Ensure HOME points to a real, writable directory before spawning Chrome.
      const home = process.env.HOME;
      const chromeEnv = {
        ...process.env,
        HOME: !home || home === '/nonexistent' ? '/tmp' : home,
      };
