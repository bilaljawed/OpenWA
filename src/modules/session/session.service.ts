...
export class SessionService implements OnModuleDestroy, OnModuleInit {
  ...
  async start(id: string): Promise<Session> {
    ...
    try {
      ...
      // Clean up so the session can be retried
      const stuckEngine = this.engines.get(id);
      if (stuckEngine) {
        await stuckEngine.destroy().catch(() => undefined);
        this.engines.delete(id);
      }
      this.cancelReconnect(id);
      await this.updateStatus(id, SessionStatus.FAILED);
      const message = error instanceof Error ? error.message.split('\n')[0] : 'Engine initialization failed';
      this.logger.error(`Session start failed: ${session.name}`, message, { sessionId: id, action: 'start_failed' });
      throw new ServiceUnavailableException(`Failed to start session: ${message}`);
    }
